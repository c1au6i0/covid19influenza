...
quint_divname_original <- extract_model(quint_divname_original, filt_cases = min_filt)
tidy(quint_divname_original)

quint_divname_original <- calculate_MRR(tidy(quint_divname_original), 10) %>%
...
quint_divname_original_nonyc <- extract_model(quint_divname_original_nonyc, filt_cases = min_filt)

quint_divname_original_nonyc <- calculate_MRR(tidy(quint_divname_original_nonyc), 10) %>%
...
quint_state_original_nonyc <- extract_model(quint_state_original_nonyc, filt_cases = min_filt)

quint_state_original_nonyc <- calculate_MRR(tidy(quint_state_original_nonyc), 10) %>%
...
tert_divname_original <- extract_model(tert_divname_original, filt_cases = min_filt)
tidy(tert_divname_original)

# look at the levaege and outlier points
TodayData <- data_to_use %>%
  filter(NC_cases >= 1) %>%
  filter(date == max(date))
# Influence index plot (hat: levarage, bonf: outlier)
# influenceIndexPlot(tert_divname_original, vars = c("hat", "Bonf"))

TodayData[c(146, 276), c("county", "state")]
TodayData[c(1034, 1458), c("county", "state")]

tert_divname_original <- calculate_MRR(tidy(tert_divname_original), 10) %>%
...
tert_divname_original_nonyc <- extract_model(tert_divname_original_nonyc, filt_cases = min_filt)

tert_divname_original_nonyc <- calculate_MRR(tidy(tert_divname_original_nonyc), 10) %>%
...
tert_state_original_nonyc <- extract_model(tert_state_original_nonyc, filt_cases = min_filt)

tert_state_original_nonyc <- calculate_MRR(tidy(tert_state_original_nonyc), 10) %>%
...
cont_divname_original <- extract_model(cont_divname_original, filt_cases = min_filt)

cont_divname_original <- calculate_MRR(tidy(cont_divname_original), 10) %>%
...
cont_divname_original_inter <- glm_pp(
  dat = data_to_use,
  dates_tostudy = max(dates_tostudy),
  filt_cases = min_filt,
  form = form_ps,
...
cont_divname_original_inter <- extract_model(cont_divname_original_inter, filt_cases = min_filt)
summary(cont_divname_original_inter)
...
cont_divname_original_nonyc <- extract_model(cont_divname_original_nonyc, filt_cases = min_filt)

cont_divname_original_nonyc <- calculate_MRR(tidy(cont_divname_original_nonyc), 10) %>%
...
cont_state_original_nonyc <- extract_model(cont_state_original_nonyc, filt_cases = min_filt)

cont_state_original_nonyc <- calculate_MRR(tidy(cont_state_original_nonyc), 10) %>%
  mutate(adjustment = "state", nyc_removed = TRUE, type_pp = "continuous")

MMR_secondary_original_binded <- bind_rows(
  quint_divname_original,
  quint_state_original,
  quint_divname_original_nonyc,
  quint_state_original_nonyc,

  tert_divname_original,
  tert_divname_original_nonyc,
  tert_state_original,
  tert_state_original_nonyc,

  cont_divname_original,
  cont_divname_original_nonyc,
  cont_state_original,
  cont_state_original_nonyc,
)
